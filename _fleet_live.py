...
NEW="10.10"
DEALS=[(d["lbl"], d["opp_id"]) for d in json.load(open("cc_work/_live_set.json",encoding="utf-8"))]
...
if __name__=="__main__":
    LIVE=live_colour()
    print(f"[{ts()}] {len(DEALS)} LIVE deals | live={LIVE} scaling to {TASKS} conc {CONC}",flush=True)
    scale(LIVE,TASKS)
    ...
    with open("cc_work/_fleet_live.csv","w",newline="",encoding="utf-8-sig") as fh:
    ...
    print("FLEET-LIVE-DONE",flush=True)
