"""Re-sweep the 80 deals that have a ticked-off (completed) to-do, FROM SCRATCH
(/update-living-memory) — same mode the completed-to-do trigger now uses. Scales blue,
conc 24. Resume-safe via a done-file. Best-effort; reports per deal."""
...
DONE = "cc_work/_todos_done.txt"
done_ids = set()
if os.path.exists(DONE):
    done_ids = set(x.strip() for x in open(DONE, encoding="utf-8") if x.strip())
ALL = json.load(open("cc_work/_completed_todo_opps.json", encoding="utf-8"))
DEALS = [o for o in ALL if o not in done_ids]
...
def rec(oid): return (requests.get(f"{SB}/rest/v1/deal_records", params={"select": SEL, "opp_id": f"eq.{oid}"}, headers=SH, verify=False, timeout=(10, 90)).json() or [{}])[0]
def scale(svc, n):
...
def mark(oid):
    with _dl:
        open(DONE, "a", encoding="utf-8").write(oid + "\n")
def one(oid):
    ...
    while time.time() - t0 < PER_TO:
        ...
        if a.get("updated_at") != bu and a.get("w") is not None:
            done[0] += 1; mark(oid); RES[oid] = {"ok": True, "a": a}
            print(f"[{ts()}] OK {done[0]}/{len(ALL)} {str(a.get('acct'))[:26]:26} win={a.get('w')} mom={a.get('m')} v{a.get('eng')}", flush=True)
            return
    done[0] += 1; RES[oid] = {"ok": False, "a": rec(oid)}
    print(f"[{ts()}] TO {done[0]}/{len(ALL)} {oid}", flush=True)
if __name__ == "__main__":
    LIVE = live_colour()
    print(f"[{ts()}] completed-todo rerun (from-scratch): {len(DEALS)} to run of {len(ALL)} | live={LIVE} scaling {TASKS}/conc {CONC}", flush=True)
    scale(LIVE, TASKS)
    with ThreadPoolExecutor(max_workers=CONC) as ex: list(ex.map(one, DEALS))
    ...
    with open("cc_work/_fleet_todos.csv", "w", newline="", encoding="utf-8-sig") as fh:
        w = csv.writer(fh); w.writerow(["opp_id", "ok", "win", "mom", "eng"])
        for oid in ALL:
            r = RES.get(oid, {}); a = r.get("a") or {}
            w.writerow([oid, r.get("ok"), a.get("w"), a.get("m"), a.get("eng")])
    print(f"\n{ok}/{len(DEALS)} completed-todo deals re-swept from-scratch. FLEET-TODOS-DONE", flush=True)
