...
# ----------------------------------------------------------------------------
$AWS         = "$HOME/Library/Python/3.9/bin/aws"
$Region      = "ap-south-1"
...
#     deploy ships the WORKING TREE and tags the image with the LOCAL HEAD, so a
#     divergent or dirty checkout silently deploys code that ISN'T on GitHub: a
#     local merge commit from `git pull`, an uncommitted edit, or a push that
#     FAILED above (only warned). That is how prod ran a commit (dfabe90) absent
#     from GitHub and 404'd /api/deal-engine/chat/async. Refuse unless the tree
#     is clean AND HEAD == origin/main. Override with -AllowDirty.
# ----------------------------------------------------------------------------
if (-not $AllowDirty -and (Get-Command git -ErrorAction SilentlyContinue)) {
    ...
    if ($dirty) {
        Write-Host $dirty
        throw "ABORT (sync guard): working tree has uncommitted changes (listed above). deploy ships the WORKING TREE, so this would deploy code that is NOT on GitHub. Commit and push, stash, or run git reset --hard origin/main first. Override with -AllowDirty."
    }
    ...
    if ($localHead -ne $originMain) {
        ...
        throw "ABORT (sync guard): local HEAD $hp does not match origin/main $op (ahead $ahead, behind $behind). deploy ships the WORKING TREE, so this would deploy code that differs from GitHub. Run git fetch origin then git reset --hard origin/main (or push your commits), then redeploy. Override with -AllowDirty."
    }
...
if (-not $SkipBuild) {
    ...
    # macOS: rsync the repo into a temp stage (excludes mirror the original), then
    # zip with the `zip` CLI (forward-slash entries; Dockerfile/buildspec at root).
    $stage = Join-Path "/tmp" "mase-src-$Tag"
    if (Test-Path $stage) { Remove-Item -Recurse -Force $stage }
    New-Item -ItemType Directory -Path $stage | Out-Null
    $rsyncArgs = @('-a',
        '--exclude=.git/','--exclude=mcp_output/','--exclude=exports/',
        '--exclude=attached_assets/','--exclude=__pycache__/','--exclude=.pytest_cache/',
        '--exclude=.local/','--exclude=.config/','--exclude=.claude/','--exclude=.aws/',
        '--exclude=.canvas/','--exclude=.agents/','--exclude=.venv/','--exclude=venv/',
        '--exclude=.mypy_cache/','--exclude=.pythonlibs/','--exclude=*.log',
        "$PSScriptRoot/", "$stage/")
    & rsync @rsyncArgs
    if ($LASTEXITCODE -ne 0) { throw "rsync staging failed (exit $LASTEXITCODE)" }

    $zip = Join-Path "/tmp" "mase-src-$Tag.zip"
    if (Test-Path $zip) { Remove-Item -LiteralPath $zip -Force }
    Push-Location $stage
    try { & zip -r -q -X $zip . } finally { Pop-Location }
...
"@
$tdFile = Join-Path "/tmp" "mase-taskdef-$Tag.json"
...
$flipFile = Join-Path "/tmp" "mase-flip-$Tag.json"
...
"@
$wtdFile = Join-Path "/tmp" "mase-worker-taskdef-$Tag.json"
