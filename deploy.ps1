...
if (-not $env:ALLOW_MANUAL_DEPLOY) {
    ...
    Write-Host ""
    Write-Host "  Break-glass only (CI is down): `$env:ALLOW_MANUAL_DEPLOY=1 ; .\deploy.ps1" -ForegroundColor DarkGray
    exit 1
}
...
# Force the never-miss Avoma engine ON: drop DEAL_SWEEP_PARALLEL_READERS from the secret
# list so the plain task-def env below (=true) wins (ECS forbids the same key in both).
