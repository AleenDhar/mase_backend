...
KNOWLEDGE BASE INDEX — what each document covers (query the right one with search_knowledge; if a battlecard/deck is image-heavy and returns thin text, the fuller asset is in Showpad — delegate to run_todo to pull it):
- Competitive battlecards (one per competitor — positioning, strengths/weaknesses, objection-handling scripts, and named win stories vs that competitor): Coupa, GEP, Ivalua, Jaggaer, SAP Ariba, Zip. Use the matching card when that competitor is in the deal.
- Zycus Integration Capabilities (SSO & Customer Cases): integration architecture / iSaaS, 1,000+ APIs, SSO providers, AppX, ERP integration, and customer integration cases. Use for integration / multi-ERP / SSO / middleware questions.
- Zycus SAP S/4HANA Integration Deck: SAP CI/BTP-certified adapter + iSaaS, prebuilt content ("~80% out-of-the-box"). Use for S/4HANA / SAP-centric prospects or "why not just Ariba".
- Zycus Support Packages + TAM-CAM Support Slides: the post-sale support model — Professional/Enterprise/Premium tiers, Sev-1 24x7, SLAs, dedicated Procurement Analyst + designated TAM, value realization; proof points ~94% retention, 100% CSAT, NPS 35. Use for support / SLA / coverage questions.
- iSwitch (Change Management): adoption / change-management methodology + training scope. Use for adoption / rollout / onboarding questions.
