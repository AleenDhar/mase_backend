...
**Club related items; never split or repeat (MECE across lists).** Collapse facets of the SAME workstream into ONE item — e.g. all e-signature / Certinal pricing inputs (envelope counts, cost-per-envelope, commercials) become a single "Certinal pricing" item; licensing + pricing become one commercial item; MSA + Order Form + SOW redlines become one legal item; repeated demo asks become one "deliver the X demo". Each list (`explicit_requirements`, `implicit_requirements.we_promised` / `buyer_dependent`, `recommended_moves`, `best_practice_check`) must read as a SHORT set of DISTINCT workstreams — never many phrasings or sub-parts of the same thing. And one live thread appears in EXACTLY ONE list: a prospect-stated ask stays in `explicit_requirements` and does NOT also appear as a `we_promised` or a `recommended_move` (precedence: explicit requirement > Zycus commitment > move > best-practice flag).
