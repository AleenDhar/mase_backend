...
Differentiation 20 · Preference 20 · Champion 15 · Exec access 15 · Competitive 15 · Business case 10 · Commercial 5.
  ...
  4.2c Exec access — THREE tiers (updated v10.8):
  (1) DIRECT — EB explicitly confirmed in MEDDPICC or verbally on an Avoma call AND had face time with Zycus. FULL credit (+1.0 → up to +15).
  (2) INDIRECT / CONFIRMED — EB demonstrably involved without direct face time: reviewed our solution/POC internally, mandated the project, or receives material through the champion. MEDDPICC confirms EB identity with indirect contact evidence. PARTIAL credit (+0.3 to +0.5), scaled by seniority and seriousness of involvement. A CEO/CIO reviewing the POC on a mandated project is meaningful executive reach even without a meeting.
  (3) POTENTIAL BY TITLE — A contact whose SF title is CFO/CIO/CPO/VP Finance/VP Procurement attends Avoma calls or appears in SF contacts, but has NOT been explicitly named as EB in MEDDPICC and NOT verbally confirmed as budget authority on any call. Award the SAME PARTIAL CREDIT as tier 2 (~+0.3) — but in the rationale ALWAYS label them as "potential economic buyer ([Name], [Title] — not yet confirmed as budget authority)." NEVER write "economic buyer engaged" for a tier-3 contact. Every gap bullet for tier 3 must include: "► Confirm [Name] as budget authority on the next call."
  Full credit reserved for tier 1 only. Tiers 2 and 3 share the same credit weight (~+0.3); they differ only in rationale labeling and the action required to upgrade.

  4.2d SIGNAL PRIORITY — call evidence vs MEDDPICC fields (v10.8): For four rubric factors, a dedicated AI call-based signal is authoritative. A MEDDPICC/CRM field can rescue a factor from the "no read" floor (−0.3) when the AI has no evidence, but CANNOT inflate above the AI's assessed level:
  - Differentiation (wt 20): primary = ai_fit_signal.tier from calls (Transformative → high · Material → mid · AI Curious → low). MEDDPICC "Identify Pain" field is secondary — cannot lift above the tier assessment.
  - Business case (wt 10): primary = ai.business_case from call evidence (quantified ROI, cost justification, named pain with a number attached). MEDDPICC "Metrics" field is secondary.
  - Commercial (wt 5): primary = medd.paper_process — active paper/pricing motion evidenced in calls (SOW, MSA, redlines, pricing discussion). MEDDPICC "Purchase Process" describes HOW the buyer procures (governance, thresholds); knowing the process exists is NOT evidence that motion is underway. It does not contribute to this factor.
  - Champion (wt 15): primary = champion_strength from Avoma (strong/developing/weak/none). If champion_strength.at_risk = true, cap champion factor at ≤0.3 regardless of MEDDPICC champion checkbox.
