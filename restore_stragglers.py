...
# Only the three NOT being handled by restore3 (Bosch/Mair/MTR). Concurrency 2 to stay under 4GB.
T=[("Gamuda","006P700000Q15OU"),("Cebu Pacific","0066700000wdNe1"),("Bandhan Bank","006P700000H55TV")]
...
def run(lbl,o):
    b=(stt(o) or {}).get("upd")
    print(f"[{ts()}] {lbl}: firing in-process (conc 2)",flush=True)
    try: requests.post(f"{API}/api/deal-engine/sweep/{o}",headers=AH,json={},verify=False,timeout=(10,2200))
    ...
    while time.time()-t0<3200:
        ...
        if s and s["upd"]!=b and s["win"] is not None:
            ...
            print(f"[{ts()}] OK {lbl}: win={s['win']} mom={s['mom']} read={s['read']!r} eng=v{s['eng']} calls={s['calls']} {'GOVERNED' if ok else 'CHECK'}",flush=True); return
    print(f"[{ts()}] FAIL {lbl}: timeout",flush=True)
with ThreadPoolExecutor(max_workers=2) as ex: list(ex.map(lambda d: run(*d), T))
print(f"[{ts()}] STRAGGLERS-DONE",flush=True)
