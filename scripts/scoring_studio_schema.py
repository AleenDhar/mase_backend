"""Scoring Version Studio — control-plane store (Supabase) + seed.

Creates `scoring_instructions` (versioned engine instructions, lock-before-run) and
`deal_outputs` (provenance stamps), then seeds the FIVE engines with their version trails
from MASE_Scoring_Studio_Staging_Handoff.md — latest version locked, priors as audit rows.

Engines: extract (Signal Extraction) · win (Win Position) · mom (Deal Momentum)
         · todo (To-Do Generation) · sum (24-Hour Summary)
Latest locked per handoff §0: extract 10.3 · win 10.3 · mom 10.1 · todo 10.1 · sum 10.1.

Idempotent: skips seeding an engine that already has rows. --apply to write; else dry-run.
DDL via the Supabase Management API (single statements); rows via PostgREST.
"""
...
DDL = [
    """create table if not exists scoring_instructions (
      ...
      engine      text not null check (engine in ('extract','win','mom','todo','sum')),
      version     text not null,
      kind        text not null check (kind in ('initial','minor','major')),
      note        text not null,
    ...
    """create index if not exists scoring_instructions_engine_idx
       on scoring_instructions (engine, locked, created_at desc)""",
    """create table if not exists deal_outputs (
      ...
      primary key (opp_id, engine, run_at)
    )""",
]

_HIST = "(historical version — full text not preserved in the handoff; the changelog note above is the record. The engine executes the latest locked version.)"

# ---------------------------------------------------------------- full latest texts (verbatim, handoff §6)
EXTRACT_V103 = """# ZYCUS SIGNAL EXTRACTION / DEAL-READING — SYSTEM INSTRUCTION · v10.0
...
WIN_V103 = """# ZYCUS WIN POSITION — SYSTEM INSTRUCTION · v10.0
...
MOM_V101 = """# ZYCUS DEAL MOMENTUM — SYSTEM INSTRUCTION · v10.0
...
TODO_V101 = """# ZYCUS TO-DO GENERATION — SYSTEM INSTRUCTION · v10.0
...
SUM_V101 = """# ZYCUS 24-HOUR SUMMARY — SYSTEM INSTRUCTION · v10.0
...
# ---------------------------------------------------------------- version trails (handoff §6)
SEED = {
    "extract": [
        ("10.0", "initial", "Initial release — founding extraction instruction (v10.0).", _HIST, False),
        ("10.1", "minor", "§A5 upgraded from name→role resolution to full FUZZY ENTITY RESOLUTION: canonical roster (Avoma attendee emails + OpportunityContactRole + account/task contacts + MEDDPICC), tiered match email → exact-name → fuzzy edit-distance + phonetic (Soundex/Metaphone) → title→person → first/last-token; DEDUPE all variants of one person; attach names to bare titles; NEVER infer a no-show from recording absence.", _HIST, False),
        ("10.2", "minor", "GOLD-MINE SOURCES promoted to a hard mandate at the head of the extraction pass: ALWAYS read Next Step + Next Step History + Completed Tasks IN FULL, including each Task Description where Avoma summaries are logged as '-- Avoma Note Start --'. Upstream enforcement feeding all four downstream engines.", _HIST, False),
        ("10.3", "minor", "TRANSCRIPT DEEP-DIVE (surgical). §A10: default to the summary; escalate to the FULL TRANSCRIPT only when a specific, material question would change a score/to-do/direction AND the summary can't answer it AND it's decision-relevant. §B1a fetch order: MASE Data Lake FIRST (avoma_transcripts by meeting_uuid → transcript_text), Avoma ONLY as fallback. Whole transcript or none; respect budgets.", EXTRACT_V103, True),
    ],
    "win": [
        ("10.0", "initial", "Initial release — founding scoring instruction (v10.0).", _HIST, False),
        ("10.1", "minor", "Exec access now credits INDIRECT economic-buyer engagement (e.g. CEO/CIO reviewing the POC internally on a mandated project) at partial strength (~+0.3 to +0.5) — previously a missing EB scored ~0/negative. Full credit still reserved for direct EB face time.", _HIST, False),
        ("10.2", "minor", "Added §5.5 Stage-reality & forecast-reality override: the engine MAY cross a stage/forecast ceiling in EITHER direction when HARD physical evidence shows the recorded stage is wrong — ONLY with (1) a written exception statement citing the evidence and the true stage, and (2) a nudge urging the seller to correct the stage/forecast in Salesforce. Symmetric. Default remains the recorded-stage ceiling.", _HIST, False),
        ("10.3", "minor", "GOLD-MINE SOURCES rule: Next Step + Next Step History + Completed Tasks (INCLUDING each Task Description with '-- Avoma Note Start --' summaries) must be read IN FULL every run — never inferred from LastActivityDate/metadata.", WIN_V103, True),
    ],
    "mom": [
        ("10.0", "initial", "Initial release — founding instruction (v10.0).", _HIST, False),
        ("10.1", "minor", "GOLD-MINE SOURCES rule: Next Step + Next Step History + Completed Tasks (INCLUDING each Task Description with '-- Avoma Note Start --' summaries) must be read IN FULL every run — never inferred from LastActivityDate/metadata.", MOM_V101, True),
    ],
    "todo": [
        ("10.0", "initial", "Initial release — founding instruction (v10.0).", _HIST, False),
        ("10.1", "minor", "GOLD-MINE SOURCES rule: read Next Step + Next Step History + Completed Tasks (INCLUDING each Task Description with '-- Avoma Note Start --' summaries) in full before generating to-dos — a call's action items, commitments and buyer asks live there.", TODO_V101, True),
    ],
    "sum": [
        ("10.0", "initial", "Initial release — founding instruction (v10.0).", _HIST, False),
        ("10.1", "minor", "GOLD-MINE SOURCES rule + read Task Descriptions (Avoma '-- Avoma Note Start --' summaries), and NEVER dismiss a recent detailed/summarized call as 'no change' — surface it under the §2 essential-change rule with an 'as of' note. Fixes the Austrian Post miss where the 1-Jul onsite summary was logged in SF but reported as 'no change'.", SUM_V101, True),
    ],
}
...
def main():
    ...
    print(f"engines: {list(SEED)} | rows to seed: {total_rows} | latest locked: "
          + ", ".join(f"{e} {v[-1][0]}" for e, v in SEED.items()))
    if not APPLY:
        print("[DRY RUN] --apply to create tables + seed."); return

    for i, stmt in enumerate(DDL, 1):
        r = requests.post(mgmt, headers={"Authorization": f"Bearer {tok}", "Content-Type": "application/json"},
                          json={"query": stmt}, verify=VERIFY, timeout=60)
        print(f"DDL {i}/{len(DDL)}: {r.status_code}" + ("" if r.status_code < 300 else f" {r.text[:200]}"))
        if r.status_code >= 300:
            return

    seeded = skipped = 0
    for engine, rows in SEED.items():
        existing = requests.get(f"{base}/rest/v1/scoring_instructions",
                                params={"engine": f"eq.{engine}", "select": "version", "limit": "1"},
                                headers=h, verify=VERIFY, timeout=30).json()
        if isinstance(existing, list) and existing:
            print(f"  {engine}: rows exist — skipping (idempotent)"); skipped += 1; continue
        payload = [{
            "engine": engine, "version": ver, "kind": kind, "note": note, "content": content,
            "locked": locked, "locked_by": "handoff-seed" if locked else None,
            "locked_at": "now()" if False else None,
        } for (ver, kind, note, content, locked) in rows]
        # locked_at via SQL default not possible on REST insert with now(); set explicitly after
        r = requests.post(f"{base}/rest/v1/scoring_instructions",
                          headers={**h, "Content-Type": "application/json", "Prefer": "return=minimal"},
                          json=payload, verify=VERIFY, timeout=60)
        if r.status_code >= 300:
            print(f"  {engine}: SEED FAILED {r.status_code} {r.text[:200]}"); return
        seeded += len(payload)
        print(f"  {engine}: seeded {len(payload)} rows (latest {rows[-1][0]} locked)")
    # stamp locked_at on locked rows
    r = requests.post(mgmt, headers={"Authorization": f"Bearer {tok}", "Content-Type": "application/json"},
                      json={"query": "update scoring_instructions set locked_at = now() where locked and locked_at is null returning engine, version"},
                      verify=VERIFY, timeout=60)
    print(f"locked_at stamped: {r.json() if r.status_code < 300 else r.text[:150]}")
    print(f"\nDONE: seeded {seeded} rows | engines skipped (already present): {skipped}")
