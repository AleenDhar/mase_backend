"""One-off / idempotent: create the 5 Analysis tables (Task #52), enable
Supabase realtime on them, and grant SELECT to anon/authenticated.

DDL via the Supabase Management API (same approach as
scripts/import_observatory.py). Safe to re-run.

Run: python3 scripts/setup_analysis_schema.py
"""
import os
import sys
...
SQL_PATH = os.path.join(_HERE, "..", "migrations", "0002_analysis.sql")
...
if __name__ == "__main__":
    with open(SQL_PATH, encoding="utf-8") as fh:
        ddl = fh.read()
    _run(ddl, "DDL (tables + indexes + grants)")
    _run(REALTIME_SQL, "realtime publication")
    print("[DONE] analysis schema ready")
