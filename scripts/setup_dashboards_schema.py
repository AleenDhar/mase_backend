"""One-off / idempotent: create the dashboards table (Task #53), enable Supabase
realtime on it, and grant SELECT to anon/authenticated.

DDL via the Supabase Management API (same approach as
scripts/setup_analysis_schema.py). Safe to re-run.

Run: python3 scripts/setup_dashboards_schema.py
"""
import os
import sys
...
SQL_PATH = os.path.join(_HERE, "..", "migrations", "0003_dashboards.sql")
...
if __name__ == "__main__":
    with open(SQL_PATH, encoding="utf-8") as fh:
        ddl = fh.read()
    _run(ddl, "DDL (table + indexes + grants)")
    _run(REALTIME_SQL, "realtime publication")
    print("[DONE] dashboards schema ready")
