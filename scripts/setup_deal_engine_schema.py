"""One-off / idempotent: create the deal_records table (Deal Intelligence
Engine), enable Supabase realtime on it, and grant SELECT to anon/authenticated.

DDL via the Supabase Management API (same approach as
scripts/setup_dashboards_schema.py). Safe to re-run.

Run: python3 scripts/setup_deal_engine_schema.py
"""
import os
import sys
...
SQL_PATH = os.path.join(_HERE, "..", "migrations", "0005_deal_engine.sql")

_TABLES = ["deal_records"]
...
if __name__ == "__main__":
    ...
    print("[DONE] deal_engine schema ready")
