"""One-off / idempotent: create the deal_hard_refresh_runs table (Deal Engine
token-free hard-fact reconciliation run history), enable Supabase realtime on the
table, and grant SELECT to anon/authenticated.

DDL via the Supabase Management API (same approach as
scripts/setup_deal_trigger_runs_schema.py). Safe to re-run.

Run: python3 scripts/setup_deal_hard_refresh_runs_schema.py
"""
import os
import sys
...
SQL_PATH = os.path.join(_HERE, "..", "migrations", "0011_deal_hard_refresh_runs.sql")

_TABLES = ["deal_hard_refresh_runs"]
...
if __name__ == "__main__":
    with open(SQL_PATH, encoding="utf-8") as fh:
        ddl = fh.read()
    _run(ddl, "DDL (table + index + grants)")
    _run(REALTIME_SQL, "realtime publication")
    print("[DONE] deal_hard_refresh_runs schema ready")
