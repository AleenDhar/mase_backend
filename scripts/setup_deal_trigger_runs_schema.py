"""One-off / idempotent: create the deal_trigger_runs table + deal_trigger_latest
view (Deal Engine analysis-run audit log), enable Supabase realtime on the table,
...
Run: python3 scripts/setup_deal_trigger_runs_schema.py
"""
import os
import sys
...
SQL_PATH = os.path.join(_HERE, "..", "migrations", "0006_deal_trigger_runs.sql")

_TABLES = ["deal_trigger_runs"]
...
if __name__ == "__main__":
    with open(SQL_PATH, encoding="utf-8") as fh:
        ddl = fh.read()
    _run(ddl, "DDL (table + view + indexes + grants)")
    _run(REALTIME_SQL, "realtime publication")
    print("[DONE] deal_trigger_runs schema ready")
