...
_LLM_PRICING: Dict[str, tuple] = {
    ...
    # Sonnet 5: $3/$15 list. NOTE an introductory $2/$10 applies through 2026-08-31, so a
    # logged cost is an UPPER BOUND until then. Added 2026-07-09: this key was MISSING, and
    # because `"claude-sonnet-4" in "claude-sonnet-5"` is False, _calculate_llm_cost fell
    # through to `return 0.0` — every sweep on DEAL_ENGINE_SWEEP_MODEL=anthropic:claude-sonnet-5
    # logged cost_usd=NULL. The cost dashboard reported those runs as FREE.
    "claude-sonnet-5":     (3.0,   15.0),
    "claude-sonnet-4":     (3.0,   15.0),   # Sonnet 4.6 / 4.5: $3/$15
...
_API_AUTH_PUBLIC_EXACT = {
    ...
    "/api/messages",
    # Teams meeting/chat tab page — Teams loads it in an iframe with no MASE token.
    "/api/teams/tab",
...
# Teams meeting/chat TAB config page. A configurable tab REQUIRES a page that runs the
# Teams JS SDK and calls setValidityState(true) so the "Save" button enables — otherwise
# adding the app to a meeting is blocked. Served from the backend (always reachable, no
# auth gate) rather than the frontend. Doubles as the tab's content (a simple MASE panel).
_TEAMS_TAB_HTML = r"""<!doctype html>
<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>MASE</title>
<script src="https://res.cdn.office.net/teams-js/2.24.0/js/MicrosoftTeams.min.js"></script>
<style>
  body{margin:0;background:#0f1115;color:#e6e8ec;font:15px/1.6 'Segoe UI',system-ui,sans-serif;padding:28px}
  .m{width:40px;height:40px;border-radius:9px;background:#4854EE;display:flex;align-items:center;justify-content:center;font-weight:800;font-size:20px;margin-bottom:14px}
  h1{font-size:20px;margin:0 0 8px}
  p{color:#c7cbd3;max-width:560px}
  code{background:#1b1f27;padding:2px 7px;border-radius:5px;color:#a9b2ff}
  ul{color:#c7cbd3;max-width:560px} li{margin:4px 0}
</style></head>
<body>
  <div class="m">M</div>
  <h1>MASE — sales-intelligence assistant</h1>
  <p>MASE is here in this chat. Just <b>@mention</b> it to get answers, right in the conversation:</p>
  <ul>
    <li><code>@MASE summarize the discussion so far</code></li>
    <li><code>@MASE what's the status of &lt;deal / account&gt;?</code></li>
    <li><code>@MASE what was the first message in this group?</code></li>
  </ul>
  <p class="muted">Replies are generated by the MASE backend.</p>
<script>
  (function () {
    try {
      microsoftTeams.app.initialize().then(function () {
        // Config context (adding the tab): enable Save + store the content url.
        if (microsoftTeams.pages && microsoftTeams.pages.config) {
          microsoftTeams.pages.config.registerOnSaveHandler(function (saveEvent) {
            microsoftTeams.pages.config.setConfig({
              entityId: "mase-panel",
              contentUrl: "https://mase-api.zycus.com/api/teams/tab",
              websiteUrl: "https://mase-api.zycus.com/api/teams/tab",
              suggestedDisplayName: "MASE"
            });
            saveEvent.notifySuccess();
          });
          microsoftTeams.pages.config.setValidityState(true);
        }
      }).catch(function () { /* opened outside Teams — just render the panel */ });
    } catch (e) { /* SDK not present — panel still renders */ }
  })();
</script>
</body></html>"""
...
try:
    ...
    @_fastapi_app.get("/api/teams/tab", response_class=HTMLResponse)
    async def _teams_tab_page():
        # Public Teams tab config/content page (loaded by Teams in an iframe). Auth is
        # not applicable — it's exempted in _API_AUTH_PUBLIC_EXACT.
        return HTMLResponse(content=_TEAMS_TAB_HTML)

    print("[STARTUP] teams control room mounted (GET /api/teams/admin, /api/teams/tab)")
except Exception as _e:  # noqa: BLE001
