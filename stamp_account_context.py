...
def main():
    ...
    for acct, rows_ in by_acct.items():
        ...
        for r in rows_:
            oid = id15(r["opp_id"])
            best_w = best_m = 0.0; best_nm = None
            for s in rows_:
                if s is r:
                    continue
                hl = (((s.get("record") or {}).get("ai") or {}).get("deal_scores") or {}).get("headline") or {}
            ...
            if {k: prev.get(k) for k in ("sibling_closed_won", "best_sibling_win", "best_sibling_mom")} != \
               {k: ctx[k] for k in ("sibling_closed_won", "best_sibling_win", "best_sibling_mom")}:
                out[oid] = ctx
            n += 1
